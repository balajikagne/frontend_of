...
function MapOrders({order}) {
    ...
    function callme()
    {
        ...
        navigate("/oderfullinfo",{state:{orderId:order._id,amount:amount,street:order.shippingAddress.street,city:order.shippingAddress.city,country:order.shippingAddress.country,Date:order.createdAt.substring(0,20),infoitem:array,cname:order.name,mNumber:order.mobNumber,Dstatus:Status,track:track,variant:variant,Dcolor:Dcolor,now:now}})
